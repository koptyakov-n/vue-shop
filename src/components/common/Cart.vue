      ...
      <transition name="slide-in-right">
        <div class="cart--window" v-show="open && localOpen">
          <div class="cart--header">
            ...
            <button class="btn cart--close" @click="closeHandler">
              <svg width="22" height="23" viewBox="0 0 22 23" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="2.59473" y="0.970901" width="26.6406" height="3.55208" rx="1" transform="rotate(45 2.59473 0.970901)" fill="white"/>
                <rect width="26.6406" height="3.55208" rx="1" transform="matrix(-0.707107 0.707107 0.707107 0.707107 18.9209 0.970901)" fill="white"/>
              </svg>
            </button>
          </div>
            ...
            <ul class="cart--list">
              ...
              >
                <div class="cart--item-pic">
                  <img class="cart--item-img" :src="item.pic" :alt="item.name">
                </div>
                ...
                <button class="btn cart--item-remove" @click="removeItem(item.productId)">
                  <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect x="2.39648" y="0.313931" width="17.7604" height="2.36806" rx="0.666667" transform="rotate(45 2.39648 0.313931)" fill="#333333"/>
                    <rect width="17.7604" height="2.36806" rx="0.666667" transform="matrix(-0.707107 0.707107 0.707107 0.707107 13.2806 0.313931)" fill="#333333"/>
                  </svg>
                </button>
              </li>
            </ul>
          ...
          <div v-if="cart.length" class="cart--footer">
            ...
            <button @click="submitHandler" class="btn cart--confirm">Оформить заказ</button>
          </div>
        </div>
      </transition>
    </div>
...
export default {
  ...
  computed: {
    total() {
      if (this.cart){
        return this.cart.reduce((value, item) => value += +item.price * +item.quantity, 0)
      }
      else {
        return 0
      }
    ...
    quantity() {
      if (this.cart){
        const cartQuantity = this.cart.reduce((quant, item) => quant += item.quantity, 0)
